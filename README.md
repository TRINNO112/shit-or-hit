...
- **🎭 Custom PNG Sticker Upload**: Upload any transparent PNG sticker, meme, or personal avatar to replace the default mascot.
- **Lossless 1080p Export**: Direct HTML5 Canvas drawing with subpixel antialiasing and lossless PNG download.
- **Seamless Segmented Controls**: Full-bleed pill controls with zero border misalignment or hairline color variations.
...
1. **💬 The Homie Letter (4 Paragraphs)**:
...
5. **Persistent Storage & Language Isolation**: Evaluated dossiers are cached in client storage and `data/reports.json` by language key (`report_real_2026_08_english`), loading instantly with zero latency.
...
[ User Interaction ] ──▶ [ Sub-1ms Optimistic UI ] ──▶ [ LocalStorage & JSON Flat File ]
                                 │
                                 └──▶ [ 4s Race Async Sync ] ──▶ [ Firebase Firestore ]
```
...
1. Open `https://trinno112.github.io/shit-or-hit/` in your mobile browser.
...
- **Frontend**: React 18, Vite 5, Tailwind CSS, Framer Motion (spring physics), Canvas Confetti, Lucide Icons.
- **Backend API Server**: Node.js, Express, Watch Mode (`node --watch`).
- **AI Synthesis Engine**: Google Gemini API (`gemini-3.5-flash-lite`).
- **Cloud Backend**: Google Firebase Auth & Firestore CDN Client.
- **Storage Engine**: Offline Local-First Engine (`localStorage`, `data/entries.json`, `data/reports.json`) + Firebase Firestore.
