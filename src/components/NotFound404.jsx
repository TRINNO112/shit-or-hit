  ...
  return (
    <div className="relative min-h-screen bg-[#FFFDF5] text-black font-sans flex flex-col justify-between p-4 md:p-8 overflow-hidden">
      ...
      {confetti.length > 0 && (
        <div className="pointer-events-none fixed inset-0 z-50 overflow-hidden" aria-hidden="true">
          {confetti.map((c) => (
            <span
              key={c.id}
              className="absolute -top-2.5 w-2.5 h-2.5 rounded-sm border border-black confetti-piece"
              style={{
                left: `${c.left}%`,
      ...
      <header className="max-w-5xl w-full mx-auto flex items-center justify-between py-2 border-b-2 border-black pb-4">
        ...
        <button
          onClick={onGoHome}
          className="flex items-center gap-2 px-4 py-2 bg-white border-2 border-black rounded-xl shadow-[3px_3px_0px_#000000] hover:shadow-[5px_5px_0px_#000000] active:translate-x-0.5 active:translate-y-0.5 font-bold text-xs uppercase cursor-pointer transition-all"
        >
          <ArrowLeft size={16} />
      ...
      <main className="max-w-3xl w-full mx-auto my-8 flex flex-col items-center text-center">
        ...
        <div className="flex flex-wrap items-center justify-center gap-4 w-full max-w-md">
          <button
            onClick={onGoHome}
            className={`flex-1 min-w-50 flex items-center justify-center gap-2 px-6 py-3.5 bg-[#FDC800] border-2 border-black rounded-2xl shadow-[4px_4px_0px_#000000] hover:shadow-[6px_6px_0px_#000000] hover:-translate-y-0.5 active:translate-x-0.5 active:translate-y-0.5 font-black uppercase text-sm tracking-wide cursor-pointer transition-all ${
