...
class SoundEngine {
  ...
  // 6. Emergency Down-Bad Ransom Capsule Unlock Alarm & Vault Release
  playCapsuleUnlock() {
    ...
    try {
      ...
      const now = this.ctx.currentTime;
      // High-impact dual pulse warning siren
      [440, 880, 587.33].forEach((freq, idx) => {
        const osc = this.ctx.createOscillator();
        const gain = this.ctx.createGain();

        osc.type = 'sawtooth';
        osc.frequency.setValueAtTime(freq, now + idx * 0.12);
        osc.frequency.exponentialRampToValueAtTime(freq * 0.85, now + idx * 0.12 + 0.18);
  ...
  // 7. Tactile Typewriter Keystroke
  playTypewriterKey() {
    if (!this.isSoundEnabled()) return;
    try {
      this.initContext();
  ...
  // 8. Authentic Thermal Printer Paper Feed Motor Sound
  playThermalPrint() {
    ...
    try {
      ...
      const now = this.ctx.currentTime;
      // Low stepper motor buzz + noise paper friction
      const osc = this.ctx.createOscillator();
      const gain = this.ctx.createGain();
